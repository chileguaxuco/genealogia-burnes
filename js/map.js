// ===== MAP.JS — Mapbox GL Map with timeline and layer filters =====

mapboxgl.accessToken = 'pk.eyJ1IjoicmljODgiLCJhIjoiY2xnemlvY2NtMDAzbzNlb2kyZ2p2ang3ayJ9.5goE20oOQgdgqLhkNeYE7Q';
...
function initMap(data) {
  ...
  window.burnesMap.addControl(new mapboxgl.NavigationControl({ showCompass: false }), 'bottom-right');
  ...
  // Setup layer toggles immediately (DOM already exists)
  setupLayerToggles();
}
...
function addMapEvents(data) {
  ...
  data.persons.forEach(p => {
    if (p.birthPlace && data.locations[p.birthPlace]) {
      events.push({
        ...
        place: p.birthPlace,
        coords: { ...data.locations[p.birthPlace] }
      });
    }
    if (p.deathPlace && data.locations[p.deathPlace]) {
      events.push({
        ...
        place: p.deathPlace,
        coords: { ...data.locations[p.deathPlace] }
      });
    }
    if (p.marriagePlace && data.locations[p.marriagePlace]) {
      events.push({
        ...
        type: 'marriage',
        label: 'Matrimonio',
        year: extractYear(p.marriageDate),
        place: p.marriagePlace,
        coords: { ...data.locations[p.marriagePlace] }
      });
    }
  });
  ...
  events.forEach(e => {
    ...
    if (count > 1) {
      ...
      e.coords = {
        lat: e.coords.lat + Math.sin(angle) * 0.06,
        lng: e.coords.lng + Math.cos(angle) * 0.06
      };
    }
  });
...
function addMigrationLines(data) {
  data.persons.forEach(p => {
    ...
    // Prefer explicit migration array if available
    if (p.migration && p.migration.length >= 2) {
      ...
      // Use person's life events for timeline
      const years = [
      ...
      // Fallback: infer from birth/marriage/death places
      const eventCoords = [];
      if (p.birthPlace && data.locations[p.birthPlace]) {
        eventCoords.push({ coords: data.locations[p.birthPlace], year: extractYear(p.birthDate) });
      }
      if (p.marriagePlace && data.locations[p.marriagePlace] && p.marriagePlace !== p.birthPlace) {
        eventCoords.push({ coords: data.locations[p.marriagePlace], year: extractYear(p.marriageDate) });
      }
      if (p.deathPlace && data.locations[p.deathPlace] && p.deathPlace !== p.birthPlace && p.deathPlace !== p.marriagePlace) {
        eventCoords.push({ coords: data.locations[p.deathPlace], year: extractYear(p.deathDate) });
      }
...
function setupLayerToggles() {
  ...
  legend.addEventListener('click', (e) => {
    ...
    if (layerFilters[layer]) {
      toggle.classList.remove('legend-off');
    } else {
      toggle.classList.add('legend-off');
    }

    applyMapVisibility(APP.currentYear);
  });

  // Also block mousedown from reaching the map
  legend.addEventListener('mousedown', (e) => { e.stopPropagation(); });
  legend.addEventListener('pointerdown', (e) => { e.stopPropagation(); });
}

// ---- Combined visibility: timeline + layer filters ----
...
function applyMapVisibility(year) {
  mapMarkers.forEach(({ element, event }) => {
    ...
    if (!typeVisible) {
      // Layer filter off: hide completely
      element.style.opacity = '0';
      element.style.transform = 'scale(0)';
      element.style.pointerEvents = 'none';
    } else if (eventYear === null) {
      // No date: always visible but subtle
      element.style.opacity = '0.4';
      element.style.transform = 'scale(0.8)';
      element.style.pointerEvents = 'auto';
    } else if (eventYear > year) {
      // Event hasn't happened yet: hidden
      element.style.opacity = '0';
      element.style.transform = 'scale(0.3)';
      element.style.pointerEvents = 'none';
    } else {
      // Event has happened: visible
  ...
  migrationLines.forEach(({ layerId, earliestYear, latestYear }) => {
    ...
    if (!visible) {
      window.burnesMap.setPaintProperty(layerId, 'line-opacity', 0);
      return;
    }

    if (earliestYear === null) {
      window.burnesMap.setPaintProperty(layerId, 'line-opacity', 0.1);
    } else if (year >= latestYear) {
      window.burnesMap.setPaintProperty(layerId, 'line-opacity', 0.3);
    } else if (year >= earliestYear) {
      window.burnesMap.setPaintProperty(layerId, 'line-opacity', 0.15);
    } else {
      window.burnesMap.setPaintProperty(layerId, 'line-opacity', 0);
    }
  });
